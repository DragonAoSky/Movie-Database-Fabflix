...
function handleListResult(resultData) {
    ...
    var checkS = resultData[0]["status"];
    if(checkS === "fail")
    {
    ...
    {
        for (let i = 1; i < Math.min(105, resultData.length); i++) {
...
//     // var tb = $(tr).parent();
//     // var id  = $(node).attr("id");
//     $.post("api/cart",{"id":obj.id},function(data){
//         if(data){
//             alert("Successfully add" + obj.id);
//         }else{
...
function add_cart(node) {
    ...
    var tb = $(tr).parent();
    var id  = $(node).attr("id");
    $.get("api/cart",{"id":id},function(data){
        if(data){
...
jQuery.ajax({
    ...
    url: "api/movie?mode=" + mode + "&browse=" + browse + "&sort=" + sort + "&page=" + page + "&num=" + num + "&title=" + title, // Setting request url, which is mapped by StarsServlet in Stars.java
